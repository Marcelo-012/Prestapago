...
class AmortizationCalculator {
  ...
  static List<Amortizacion> recalcularPorMora({
    ...
    double saldo = pendientes.fold(0.0, (sum, a) => sum + a.montoCapital) + montoMora;
  ...
  static ({List<Amortizacion> actualizadas, List<int> idsCanceladas}) recalcularPorAbonoCapital({
    required List<Amortizacion> pendientes,
    ...
    if (pendientes.isEmpty) return (actualizadas: pendientes, idsCanceladas: []);

    final tasaMensual = _toMonthlyRate(tasaInteres, periodicidadIntereses);
    double saldo = pendientes.fold(0.0, (sum, a) => sum + a.montoCapital) - abono;
    if (saldo < 0) saldo = 0;
  ...
  static ({String tipo, double saldoFavorRestante, double diferenciaACubrir}) recalcularPorSaldoAFavor({
    ...
    if (saldoAFavorAcumulado == cuotaActual) {
      ...
      return (tipo: 'mayor', saldoFavorRestante: saldoAFavorAcumulado - cuotaActual, diferenciaACubrir: 0);
    } else {
      return (tipo: 'menor', saldoFavorRestante: 0, diferenciaACubrir: cuotaActual - saldoAFavorAcumulado);
    }
  }
  ...
  static List<Amortizacion> calcular({
    ...
    final tasaMensual = _toMonthlyRate(tasaInteres, periodicidadIntereses);
    final tasaMoraMensual = _toMonthlyRate(tasaInteresMoratoria, periodicidadIntereses);
    double saldo = monto;
    int idCuota = 0;
